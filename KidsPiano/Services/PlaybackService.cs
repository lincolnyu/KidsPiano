...
public class PlaybackService : IDisposable
{
    ...
    private async Task PlayMeasureNotes(Measure measure, CancellationToken token)
    {
        ...
        foreach (var n in measure.Notes)
        {
            ...
            var velocity = (int)(Volume / 100.0 * 100 + 27); // map 0-100 → 27-127
        ...
        async Task WaitUntil(double delay)
        {
            ...
            if (delayMs > 50) await Task.Delay(delayMs, token);
        }
    }
    ...
    private class NotesComparerByEnd : IComparer<Note>
    {
        public static readonly NotesComparerByEnd Instance = new();
